...
Client Request: Client sends POST /api/orders to YARP API Gateway (:5000)
Order Creation: Gateway routes to Ordering.API, which persists the order to OrderingDb
Event Publish: Ordering.API publishes OrderSubmittedEvent to RabbitMQ and returns 200 OK to the client immediately
Event Consume: Delivery.API pulls OrderSubmittedEvent from RabbitMQ in the background, creates a delivery record, and assigns a driver
📂 Project Structure
FoodDelivery/
├── src/
│   ├── Gateways/
...
cd FoodDelivery
Build and run all 7 containers in background mode:

docker compose up -d
Stream live events between publisher and consumer:
